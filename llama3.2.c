...
typedef struct {
    int dim; // hidden_size (2048)
    int hidden_dim; // intermediate_size (8192)
    int n_layers; // num_hidden_layers (16)
    int n_heads; // num_attention_heads (32)
    int n_kv_heads; // num_key_value_heads (8)
    int vocab_size; // vocab_size (12856)
    int seq_len; // max_position_embeddings (131072)
} Config;
...
typedef struct {
    ...
    // (vocab_size, dim) (12856, 2048)
    
    // weights for rmsnorms
    float* rms_att_weight; 
    // (layer, dim) (16,2048) rmsnorm weights
    // model.layers.i.input_layernorm.weight
    float* rms_ffn_weight; 
    // (layer, dim) (16, 2048) post-attention rmsnorm weight
    ...
    // weights for ffn
    float* w1; 
    // (layer, hidden_dim, dim) SwiGLU gate
    // model.layers.i.mlp.gate_proj.weight
    float* w2; 
    // (layer, dim, hidden_dim) project to hidden dim
    // model.layers.i.mlp.down_proj.weight
    float* w3; 
    // (layer, hidden_dim, dim) project to immediate dim
